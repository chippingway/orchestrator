...
"""The two structured verdicts a stage handler is allowed to act on.

Both parsers are deliberately strict: the marker shape is fixed, the LAST
occurrence wins so a template line quoted earlier in a long message cannot
override the concluding one, and anything short of a real marker resolves to
`unknown` -- the branch on which the caller parks a human in instead of
recording an outcome the agent never committed to.
"""
