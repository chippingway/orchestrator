...
class _PublishedRaceCase(
    ...
    def _settles_while(self, arrives) -> object:
        ...
        with raced, watched as transport:
            settled = _Settled(self._settle(), transport)
        return settled
...
class _Settled:
    """One settlement's outcome, beside the transport it did or did not reach.

    A pair rather than two returns, because every case here asks both: what
    the record says afterwards, and whether the push happened at all.
    """

    def __init__(self, outcome, transport) -> None:
    ...
    def test_an_ending_in_the_window_pushes_nothing(self) -> None:
        for ending in _ENDINGS:
            with self.subTest(ending=ending):
                ...
                settled = self._settles_while(getattr(self, ending))

                settled.transport.assert_not_called()
                self._assert_unpublished(settled.outcome)
