...
_FLAT_MODULES = (
    "orchestrator.stages._validating_export_manifest",
    "orchestrator.stages._validating_exports",
...
def _stage_targets() -> list:
    ...
    return [
        ...
        if "validating" in target.module_name
    ]
...
class OwnerImportSiteTest(unittest.TestCase):
    ...
    def test_no_flat_module_exists(self) -> None:
        ...
        # `review_round` counter, the park reasons, and the watermark keys live
        # issues are already carrying, free to drift from the owner silently
        # and invisible to a patch aimed at it. Resolving the spec rather than
