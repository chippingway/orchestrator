...
from orchestrator.stages import (
    _implementing_export_manifest as _forwarder_manifest,
    implementing as _forwarder,
)
...
class PackageSurfaceTest(unittest.TestCase):
    ...
    def test_no_flat_leaf_is_left_behind(self) -> None:
        ...
        # off an owner here; an `_implementing_*` or `_implement_state` target
        # left in the inventory would be a leaf the flat package still owns.
        for target in _forwarder_manifest.EXPORTS:
            with self.subTest(name=target.export_name):
                self.assertNotIn("_implementing_", target.module_name)
                self.assertNotIn("_implement_state", target.module_name)
