...
_FINISHED = MappingProxyType({
    ...
    for ended, over in (
        ...
        ("its pull request", _ENDINGS["closed"]),
        ("a merge of it", _ENDINGS["merged"]),
    )
...
class TerminalWorkReconciliationTest(
    ...
    def test_the_debt_is_left_for_the_terminal(self) -> None:
        # Handing the tick back costs nothing here, and it is the whole reason
        # the door reads rather than parks: the stage's own terminal is the
        # next thing that runs, and it drains the issue with the record, the
        # branch and the debt left exactly as they are for it.
        github, issue = _owing(self)
        _ended(github)

        dispatched = self._route(github, issue)[0]
