...
def build_usage_metrics() -> _metrics.UsageMetrics:
    return _metrics.UsageMetrics(
...
def build_agent_trajectory() -> _records.AgentTrajectory:
    return _records.AgentTrajectory(
        backend=_usage_cases.CLAUDE,
        tools=(_usage_cases.BASH_TOOL, _usage_cases.READ_TOOL),
        skills=_skills.SkillTriggers(
