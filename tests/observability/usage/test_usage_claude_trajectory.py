...
class ClaudeTrajectoryTest(unittest.TestCase):
    """``_trajectory.parse_claude_trajectory`` over synthetic ``stream-json`` runs.
    ...
    def test_skips_empty_or_nonstring_text_blocks(self) -> None:
        ...
        self.assertEqual(_trajectory.parse_claude_trajectory(stdout).steps, ())
    ...
    def test_partial_frames_dedup_calls_and_results(self) -> None:
        ...
        # ``_skills.parse_claude_skills`` applies. Distinct ids stay distinct.
        stdout = _jsonl.jsonl(
            _claude.assistant(
                content_blocks=[
                    _jsonl.tool_use(
    ...
    def test_empty_stdout(self) -> None:
        self.assertEqual(_trajectory.parse_claude_trajectory(""), _records.AgentTrajectory(backend=_usage_cases.CLAUDE))
