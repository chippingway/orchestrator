...
"""The header row where every column is also the matrix's sort control.
...
filters the matrix was narrowed by live in the page it would leave behind.
...
_ARROW_SPAN = '<span class="orch-skillmatrix-sort">'
...
def _unsorted_header_html() -> str:
    """The row a matrix nobody has clicked yet is headed by."""
    return skill_matrix_headers.skill_matrix_header_html(None, False)
...
class SkillMatrixActiveHeaderTest(unittest.TestCase):
    """What the row says once one column is the one the rows are in."""

    def test_only_the_active_column_carries_an_arrow(self) -> None:
        markup = skill_matrix_headers.skill_matrix_header_html(
            _SORT_KEY_RUNS, True,
        )
        self.assertEqual(markup.count(_ARROW_SPAN), 1)

    def test_the_active_column_shows_and_reverses(self) -> None:
        descending = skill_matrix_headers.skill_matrix_header_html(
            _SORT_KEY_RUNS, True,
        )
        self.assertIn(_ARROW_DOWN, descending)
        self.assertIn(_link(_SORT_KEY_RUNS, _ASCENDING), descending)
        ascending = skill_matrix_headers.skill_matrix_header_html(
