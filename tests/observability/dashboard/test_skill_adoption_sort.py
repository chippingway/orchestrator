...
# The rate the shared cell case carries: the skill was available to four
# sessions and loaded in one.
_CELL_RATE = 0.25


class ParseSkillAdoptionSortTest(unittest.TestCase):
    """What one page URL is read back as."""

    def test_a_named_column_reads_its_direction(self) -> None:
        cases = (
            ({_SORT_PARAM: _SORT_KEY_SESSIONS}, (_SORT_KEY_SESSIONS, False)),
            (
                {_SORT_PARAM: _SORT_KEY_SESSIONS, _DIR_PARAM: _DESCENDING},
                (_SORT_KEY_SESSIONS, True),
            ),
            (
                {_SORT_PARAM: _SORT_KEY_SESSIONS, _DIR_PARAM: _ASCENDING},
                (_SORT_KEY_SESSIONS, False),
            ),
            (
            ...
            (
                {_SORT_PARAM: _SORT_KEY_INCIDENTAL, _DIR_PARAM: _DESCENDING},
                (_SORT_KEY_INCIDENTAL, True),
            ),
        )
        for query_params, expected in cases:
            with self.subTest(params=query_params):
                self.assertEqual(
                    skill_adoption_sort.parse_skill_adoption_sort(query_params),
                    expected,
                )
    ...
    def test_the_parameters_are_passed_by_name(self) -> None:
        self.assertEqual(
            skill_adoption_sort.parse_skill_adoption_sort(
                params={_SORT_PARAM: _SORT_KEY_SESSIONS},
            ),
            (_SORT_KEY_SESSIONS, False),
        )


class SkillAdoptionOrderTest(unittest.TestCase):
