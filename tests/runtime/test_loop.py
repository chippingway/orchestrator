...
from orchestrator.runtime import loop, self_update, ticks
...
def _options(once: bool) -> PollingOptions:
    return PollingOptions(once=once, log_level="INFO")


class StopAfterPasses:
    """`run_tick` stand-in that stops the run after a bounded pass count."""

    def __init__(self, state: RuntimeState, passes: int) -> None:
        self._state = state
        self._passes = passes
        self.completed = 0
