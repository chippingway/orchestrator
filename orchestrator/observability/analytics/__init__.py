...
``recording`` and ``trajectories``, the pair the orchestrator process itself
runs. Beside them sit the owners neither sink may answer separately: ``config``,
the environment knobs all of them are configured by, and ``retention`` over
``retention_scan`` / ``retention_rewrite``, the by-age prune that bounds both
JSONL files -- each on its own path, retention knob, and lock, but through one
scan and one rewrite, so the two cannot disagree about what an expired or
malformed record costs.
