...
"""The `WHERE` clause one filter set becomes, per table it is scanned against.

One builder answers for all three targets, so the raw events table, the
agent-run view, and the daily rollup cannot disagree about what a filter means:
the base table binds the window against `ts`, the rollup against its `day`
key with the bounds projected to dates, and the view drops the `events`
selection its columns cannot carry. What changes between them is the time
column and which selections survive -- never the three-case reading of a
multiselect, which is the contract the dashboard's widgets move together by.
"""
...
def build_where(
    ...
    *,
    ...
    start = day_bound(filters.start) if day_bounds else filters.start
    end = day_bound(filters.end) if day_bounds else filters.end
...
def build_window_where(
    ...
    - An empty sequence emits a tautologically-false predicate
      ...
      dimension"; encoding that as SQL is what makes summary /
    ...
    """
    return build_where(filters, time_column="ts", day_bounds=False)


def build_view_window_where(
    filters: WindowFilters,
) -> tuple[str, list[Any]]:
    """`build_window_where` minus the ``events`` clause.

    Use against `analytics_agent_runs` queries. Callers must have
    already short-circuited on `agent_event_excluded(events)` so
    the event-filter contract is honored before the SQL is built.
    """
    return build_window_where(filters.without_events())


DAILY_ROLLUP_VIEW = "analytics_daily_rollup"


def build_rollup_window_where(
    filters: WindowFilters,
) -> tuple[str, list[Any]]:
    """`build_window_where` translated to the rollup's `day` column.
    ...
    ``events`` / ``stages`` semantics mirror `build_window_where`:
    ...
    """
    return build_where(filters, time_column="day", day_bounds=True)
