...
runs reached for that skill, and the reason the panel exists is the cohort that
reached for it on none of them: the skill is in the repository's catalog and
the cohort ran, so an explicit zero is the answer an operator came for. That
zero is toned down rather than dropped, and its derived rate is toned with it,
so a row an operator is scanning past reads as quiet in both columns at once
while the cohort's own run total stays a plain number -- it is the denominator
the zero is read against, not part of the finding.

The rate is rounded to whole points, since the column is compared down the
table rather than read off a single row. A category the sink recorded nothing
for is labelled through the same `unknown` the aggregate panel reads a missing
role or backend under, so the two tables bucket an empty category the same way.

Every naming column arrives off the sink rather than out of this repository, so
all four are escaped into the markup a browser is asked to interpret.
"""
...
def skill_matrix_row_view(row: SkillTriggerMatrixRow) -> SkillMatrixRowView:
    """Reduce one cell to the readings its seven columns are drawn from."""
