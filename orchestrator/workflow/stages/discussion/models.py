...
issue and what the command line actually was cannot disagree. It is a plain
carrier because which identity applies is a property of the issue, not of the
class: `run` reads the pinned one back when there is one, and only an issue
that has never spawned falls through to the current config.
