...
pull-request reads in ``claims``. The widening of that scan with what the
remote still carries lives in ``discovery``. Every worktree name is defined on
one of these owners, and callers import the owner they need directly, so this
...
``discovery``, ``evidence``, ``inventory``, ``probes``, and ``terminal`` name
