...
"""Worktree naming, creation, recovery, and decomposer-lifecycle owners.
...
decomposer's scratch checkout lifecycle lives in ``decomposition``. Callers
import the owner they need directly, so this initializer binds nothing and
importing one owner never drags the others in.
``orchestrator.worktree_lifecycle`` stays the historical facade for callers
that reach these helpers through the worktree compatibility surface.
"""
