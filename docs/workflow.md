...
`orchestrator/workflow/stages/` one stage at a time — `decomposition` and `implementing` have gone, so the
`decomposing` / `ready` / `blocked` / `umbrella` handlers live on owners in
`orchestrator/workflow/stages/decomposition/` and `_handle_implementing` on owners in
`orchestrator/workflow/stages/implementing/`. The module each migrated stage vacates stays behind as a temporary
forwarder onto those owners, so both import sites keep handing back the same handler; the dispatcher and the same-tick
pickup start name the owner directly. The per-stage
